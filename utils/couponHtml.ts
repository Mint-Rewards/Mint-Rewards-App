...
/**
 * Builds the printable coupon — the redemption *mechanism* for a Deal.
 *
 * "Coupon" is correct throughout this file: under docs/VOCABULARY.md a coupon
 * is the code/voucher a user redeems, never the incentive itself. The
 * incentive is the Deal passed in.
 */
export function buildCouponHtml(
  ...
  // A Discount-type deal states a percentage; a fixed-amount one states the
  // amount; anything else falls back to the deal's own title.
  const discountSub = discountNum
    ? "off your next order"
    : item.discountAmount != null
      ? `Rs ${item.discountAmount} off`
      : item.title || "Exclusive Member Deal";

  const minimumLine =
    item.minimumPurchase != null
      ? `Minimum purchase Rs ${item.minimumPurchase}. `
      : "";
...
<div class="coupon">
  ...
  <header class="hero">
    ...
    <div class="offer-block">
      <div class="offer-eyebrow">EXCLUSIVE MEMBER DEAL</div>
      ${discountNum
  ...
  <div class="seal-anchor">
    <div class="seal">
      ${logoHtml}
      <div class="seal-body">
        <div class="seal-label">DEAL FROM</div>
  ...
  <section class="body">
    ...
    <p class="terms">
      ${minimumLine}One use per Mint Rewards member, per order. Excludes gift cards and sale items.
